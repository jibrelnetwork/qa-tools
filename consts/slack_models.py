...
from marshmallow import fields, Schema, pre_load, post_load, pre_dump
...
class _EnvironmentConfig(fields.Field):
...
class EnvironmentConfig(Schema):
    env_conf = fields.Dict(keys=_EnvInfo(), values=_EnvironmentConfig())

    @pre_dump
    def wrap_with_envelope(self, data, many, **kwargs):
        return {'env_conf': data}

    @post_load
    def to_default_dict(self, in_data, **kwargs):
        in_data = in_data['env_conf']
...
if __name__ == '__main__':
    ...
    keks = subs.load(env_conf)
    print(type(keks[EnvInfo('123', '123', 'qwe2', 'name1')]['services']))
    print('123')
