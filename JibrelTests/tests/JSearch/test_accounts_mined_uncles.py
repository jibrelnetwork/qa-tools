...
def test_accounts_mined_uncles_positive(address, limit, offset, order, setup):
    ...
    for item in response['data']:
        block_uncles = reference.block_uncles(item['blockNumber'])
        expected = None
        for uncle in block_uncles:
            if uncle['hash'] == item['hash']:
                expected = uncle
                break

        assert expected  # uncle not found
        assert address == Web3.toChecksumAddress(expected['miner'])
        # FIXME uncomment when uncle will be with correct values
        # assert validate.uncle(item, expected)

    actual_numbers = [item['number'] for item in response['data']]
    get_numbers = partial(reference.etherscan.get_mined_uncles_numbers,
...
def test_accounts_mined_uncles_no_data(address, limit, offset, order, setup):
    jsearch, reference, validate = setup
...
def test_accounts_mined_uncles_bad_value(address, limit, offset, order, errors, setup):
    jsearch, reference, validate = setup
