...
class NodeToJsearchDataConverter:
    ...
    def receipt(self, original):
        return self._convert(original, ("blockNumber", 'cumulativeGasUsed', 'gasUsed', 'status', "transactionIndex"))

    def receipt_logs(self, original):
        return self._convert(original, ("blockNumber", 'logIndex', "transactionIndex"))
...
class Validator:
    ...
    def receipt(self, actual, expected):
        expected_ = self.convert.receipt(expected)
        if expected_['logs']:
            logs_ = []
            for item in expected_['logs']:
                logs_.append(self.convert.receipt_logs(item))
            expected_['logs'] = logs_

        if expected_["contractAddress"] is None:
            expected_["contractAddress"] = "0x0000000000000000000000000000000000000000"

        act_only = {'root'}
        return self.validate_data_dict(actual, expected_, act_only)
